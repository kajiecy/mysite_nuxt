        ...
        <div class="left-info-fix " :class="leftMenuState?'menu_open':'menu_close'">
          <div class="overlay" @click="overLayClick">
            <div @click.stop="leftBodyClick" class="left-info-body">
              ...
              <span class="blog-function-list-item " :class="$route.name!=='index-BlogContent'||$route.query.issueNumber!=$store.state.aboutIssuesId?'active':''" @click="$router.push('/')" title="我的首页">
                                <div class="list-item-content"><i class="item-icon iconfont icon-shouye"></i>我的首页</div>
                            </span>
              <span class="blog-function-list-item " :class="$route.name==='index-BlogContent'&&$route.query.issueNumber==$store.state.aboutIssuesId?'active':''" @click="$router.push({name:'index-BlogContent',query:{issueNumber:$store.state.aboutIssuesId}})" title="关于博客">
...
<script lang="ts">
  ...
  export default class Home extends Vue {
    ...
    mounted() {
      // @ts-ignore
      this.$githubApi.getBloggerInfo().then((bloggerInfo:any)=>this.bloggerInfo = bloggerInfo);
      // @ts-ignore
